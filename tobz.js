...
module.exports = tobz = async (tobz, message) => {
    try {
        ...
        // FUNCTION
                ...
                if(body === '!botoff' && isMuted(chatId) == true){
                    if(isGroupMsg) {
                        if (!isAdmin) return tobz.reply(from, 'Maaf, perintah ini hanya dapat dilakukan oleh admin BOT-ISALL🤖!', id)
                        ...
                        tobz.reply(from, 'Bot telah di OFF pada chat ini! !boton untuk ON BOT!', id)
                    }else{
                        if(isMsgLimit(serial)){
                            return
                        }else{
                        ...
                        muted.push(chatId)
                        fs.writeFileSync('./lib/database/muted.json', JSON.stringify(muted, null, 2))
                        reply(from, 'Bot telah di OFF pada chat ini! !boton untuk ON BOT!', id)
                    }
                }
                if(body === '!boton' && isMuted(chatId) == false){
                    if(isGroupMsg) {
                        if (!isAdmin) return tobz.reply(from, 'Maaf, perintah ini hanya dapat dilakukan oleh admin BOT-ISALL🤖!', id)
                        if(isMsgLimit(serial)){
                            return
                ...
                if (body === '!unbanchat') {
                    if (!isOwner) return tobz.reply(from, 'Maaf, perintah ini hanya dapat dilakukan oleh Owner BOT-ISALL🤖!', id)
        ...
        case '!banchat':
            ...
            if (!isOwner) return tobz.reply(from, 'Perintah ini hanya bisa di gunakan oleh Owner BOT-ISALL🤖!', id)
            setting.banChats = true
            banChats = true
            fs.writeFileSync('./lib/database/setting.json', JSON.stringify(setting, null, 2))
            tobz.reply('Global chat has been on!')
            break
...
${desc}`)
        ...
        case '!owner':
        case '!creator':
            tobz.sendContact(chatId, `6289630267618@c.us`)
        ...
        case '!simi':
            if (!isGroupMsg) return tobz.reply(from, 'Perintah ini hanya bisa di gunakan dalam group!', id)
            if (!isAdmin) return tobz.reply(from, 'Perintah ini hanya bisa di gunakan oleh Admin BOT-ISALL🤖!', id) // Hanya Admin yang bisa mengaktifkan
            if (args.length === 1) return tobz.reply(from, 'Pilih on atau off!', id)
            if (args[1].toLowerCase() === 'on') {
                simi_.push(chat.id)
...
🌐️ *URL*: ${url}`
        ...
        case '!ytmp4':
            ...
            break
        case '!play':
            if (!isAdmin) return tobz.reply(from, `Perintah ini hanya bisa di gunakan oleh Admin BOT-ISALL🤖!`, id)
            if (!isGroupMsg) return tobz.reply(from, 'Perintah ini hanya bisa di gunakan dalam group', id)
...
➸ *Wilayah:* +${region_name}`
        ...
        // ADMIN & OWNER
        case '!bc':
            if (!isOwner) return tobz.reply(from, 'Perintah ini hanya untuk Owner BOT-ISALL🤖!', id)
            let msg = body.slice(4)
            const chatz = await tobz.getAllChatIds()
            for (let ids of chatz) {
                var cvk = await tobz.getChatById(ids)
                if (!cvk.isReadOnly) await tobz.sendText(ids, `[ BOT-ISALL🤖 PENGUMUMAN ]\n\n${msg}`)
            }
        ...
        case '!ownergroup':
            ...
            break
        case '!otagall': // FOR OWNER & ADMIN ISALL
        case '!omentionall':
            if (!isGroupMsg) return tobz.reply(from, 'Perintah ini hanya bisa di gunakan dalam group!', id)
            if (!isOwner, !isAdmin) return tobz.reply(from, 'Perintah ini hanya untuk Owner BOT-ISALL🤖', id)
        ...
        case '!okickall':
            if (!isGroupMsg) return tobz.reply(from, 'Perintah ini hanya bisa di gunakan dalam group!', id)
            if (!isOwner) return tobz.reply(from, 'Perintah ini hanya untuk Admin BOT-ISALL🤖', id)
            if (!isBotGroupAdmins) return tobz.reply(from, 'Perintah ini hanya bisa di gunakan ketika bot menjadi admin', id)
        ...
        case '!oadd':
            ...
            if (!isOwner, !isAdmin) return tobz.reply(from, 'Perintah ini hanya untuk Admin BOT-ISALL🤖', id)
            if (!isBotGroupAdmins) return tobz.reply(from, 'Perintah ini hanya bisa di gunakan ketika bot menjadi admin', id)
            try {
        ...
        case '!oleave':
            if (!isGroupMsg) return tobz.reply(from, 'Perintah ini hanya bisa di gunakan dalam group', id)
            if (!isOwner, !isAdmin) return tobz.reply(from, 'Perintah ini hanya untuk Admin BOT-ISALL🤖', id)
            await tobz.sendText(from,'BOT-ISALL🤖 DIPERINTAHKAN KELUAR OLEH OWNER!!').then(() => tobz.leaveGroup(groupId))
            break
        case '!leave':
            if (!isGroupMsg) return tobz.reply(from, 'Perintah ini hanya bisa di gunakan dalam group', id)
            if (!isGroupAdmins) return tobz.reply(from, 'Perintah ini hanya bisa di gunakan oleh admin group', id)
        ...
        case '!getses':
            ...
            break
        case '!isalladmin':
            let admn = `This is list of BOT-ISALL🤖 Admin\nTotal : ${adminNumber.length}\n`
        ...
        case '!addadmin':
            if (!isOwner) return tobz.reply(from, 'Perintah ini hanya bisa di gunakan oleh Owner BOT-ISALL🤖!', id)
                ...
                tobz.reply(from, 'Success Menambahkan Admin BOT-ISALL🤖!', id)
                }
            break
        case '!deladmin':
            if (!isOwner) return tobz.reply(from, 'Perintah ini hanya bisa di gunakan oleh Owner BOT-ISALL🤖!', id)
                ...
                fs.writeFileSync('./lib/database/admin.json', JSON.stringify(adminNumber))
                tobz.reply(from, 'Success Menghapus Admin BOT-ISALL🤖!', id)
            break
        case '!block':
            if (!isOwner) return tobz.reply(from, 'Perintah ini hanya bisa di gunakan oleh Owner BOT-ISALL🤖!', id)
            for (let i = 0; i < mentionedJidList.length; i++) {
        ...
        case '!help':
            ...
            break
        case '!isallgroup':
            tobz.reply(from, `Link Group BOT-ISALL🤖 : https://chat.whatsapp.com/HXoDilHy4TsJghPDltH2VL\nJangan Lupa Join Ya Kak ${pushname}`, id)
            break
        case '!groupmenu':
        ...
        case '!iklan':
            ...
            break
        case '!adminmenu':
            if (!isAdmin) return tobz.reply(from, 'Perintah ini hanya untuk Admin BOT-ISALL🤖', id)
        ...
        case '!snk':
            ...
            break
        default:
            if (!isGroupMsg) return tobz.reply(from, 'Jika Ingin Menggunakan Bot Harap Masuk Ke Dalam Grup BOT-ISALL🤖, Link Ada Di Bio atau Bisa Mengetik #isallgroup!\nJika Ingin Sewa Bot atau Bikin Bot Harap Ketik *#iklan*', id)
            if (command.startsWith('!')) {
